...
from planqk.qiskit.providers.braket.planqk_aws_quantum_task import PlanqkAwsQuantumTask


class PlanqkAwsQuantumTaskBatch:
        ...
        **aws_quantum_task_kwargs,
        ...
        Args:
            ...
            inputs ([Dict[str, float]]): Inputs to be passed along with the
                ...
                Default: {}.
        """
        self._tasks = PlanqkAwsQuantumTaskBatch._execute(
            device_arn,
            task_specifications,
            shots,
            max_parallel,
    ...
    def _execute(
        ...
        **kwargs,
        ...
        tasks_and_inputs = PlanqkAwsQuantumTaskBatch._tasks_and_inputs(task_specifications, inputs)
        ...
        try:
            with ThreadPoolExecutor(max_workers=max_threads) as executor:
                task_futures = [
                    executor.submit(
                        PlanqkAwsQuantumTaskBatch._create_task,
                        remaining,
                        device_arn,
                        task,
                        shots,
    ...
    def _create_task(
        ...
        **kwargs,
        ...
        task = PlanqkAwsQuantumTask.create(
            device_arn,
            task_specification,
            shots,
            poll_interval_seconds=poll_interval_seconds,
    ...
    def results(
        ...
        if not self._results or not use_cached_value:
            self._results = PlanqkAwsQuantumTaskBatch._retrieve_results(self._tasks, self._max_workers)
            self._unsuccessful = {
    ...
    def retry_unsuccessful_tasks(self) -> bool:
        ...
        if not unsuccessful_indices:
            return True
        retried_tasks = PlanqkAwsQuantumTaskBatch._execute(
            self._aws_session,
            self._device_arn,
        ...
        retried_results = PlanqkAwsQuantumTaskBatch._retrieve_results(retried_tasks, self._max_workers)

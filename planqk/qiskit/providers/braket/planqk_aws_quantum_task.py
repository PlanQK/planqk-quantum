...
class PlanqkAwsQuantumTask(QuantumTask):
    ...
    def create(
        ...
        **kwargs,
        ...
        """PlanqkAwsQuantumTask factory method that serializes a quantum task specification
        (either a quantum circuit or annealing problem), submits it to Amazon Braket,
        and returns back an PlanqkAwsQuantumTask tracking the execution.
        ...
        Returns:
            PlanqkAwsQuantumTask: PlanqkAwsQuantumTask tracking the task execution on the device.
        ...
        create_task_kwargs = _create_common_params(
            device_arn,
            shots if shots is not None else PlanqkAwsQuantumTask.DEFAULT_SHOTS,
        )
    ...
    def __init__(
        ...
        Examples:
            >>> task = PlanqkAwsQuantumTask(arn='task_arn')
            >>> task.state()
            'COMPLETED'
            >>> result = task.result()
            AnnealingQuantumTaskResult(...)

            >>> task = PlanqkAwsQuantumTask(arn='task_arn', poll_timeout_seconds=300)
            >>> result = task.result()
            GateModelQuantumTaskResult(...)
        """
    ...
    async def _wait_for_completion(
        ...
        Returns:
            Union[GateModelQuantumTaskResult, AnnealingQuantumTaskResult]: If the task is in the
                `PlanqkAwsQuantumTask.RESULTS_READY_STATES` state within the specified time limit,
                the result from the S3 bucket is loaded and returned.
                `None` is returned if a timeout occurs or task state is in
                `PlanqkAwsQuantumTask.NO_RESULT_TERMINAL_STATES`.
        Note:
        ...
        while (time.time() - start_time) < self._poll_timeout_seconds:
            ...
            if task_status in PlanqkAwsQuantumTask.RESULTS_READY_STATES:
                return self._download_result()
            elif task_status in PlanqkAwsQuantumTask.NO_RESULT_TERMINAL_STATES:
                self._result = None
                return None
            else:
        ...
        self._logger.warning(
            ...
            + "this can be done by creating a new PlanqkAwsQuantumTask with this task's ARN "
    ...
    def _download_result(
        ...
        result_string = self._aws_session.retrieve_s3_object_body(
            ...
            current_metadata["outputS3Directory"] + f"/{PlanqkAwsQuantumTask.RESULTS_FILENAME}",
        )
    ...
    def __repr__(self) -> str:
        return f"PlanqkAwsQuantumTask('id/taskArn':'{self.id}')"

    def __eq__(self, other) -> bool:
        if isinstance(other, PlanqkAwsQuantumTask):
...
def _(
    ...
    **kwargs,
    ...
    return PlanqkAwsQuantumTask(task_arn, aws_session, *args, **kwargs)

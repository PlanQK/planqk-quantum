...
from planqk.qiskit.providers.braket.planqk_aws_quantum_task_batch import PlanqkAwsQuantumTaskBatch
...
class _PlanqkAwsDevice(Device):
    ...
    def __init__(self, arn: str, planqk_client: _PlanqkClient):
        """
        Args:
    ...
    def run_batch(
        ...
        **aws_quantum_task_kwargs,
        ...
        """
        return PlanqkAwsQuantumTaskBatch(
            self._arn,
            task_specifications,
            # s3_destination_folder
            # or (
    ...
    def _populate_properties(self) -> None:
        metadata = self._planqk_client.get_backend_config(self._arn)
        self._name = metadata.get("name")
        self._status = metadata.get("status")
        self._type = AwsDeviceType(metadata.get("type"))
        self._provider_name = metadata.get("hardwareProvider")
        self._properties = json.dumps(metadata.get("configuration"))
        self._frames = None
        self._ports = None
    ...
    def get_devices(
        ...
        devices = planqk_client.get_backends()

        # filter be arns, names, types, statuses, provider_names
        filtered_devices = list(filter(
            lambda device_summary: (
                (not arns or device_summary['id'] in arns) # Ids are represented by arns
                and (not names or device_summary['name'] in names)
                and (not types or device_summary['type'] in types)
                and (not statuses or device_summary['status'] in statuses)
                and (not provider_names or device_summary['provider'] in provider_names)
            ),
            devices,
        ))
        ...
        device_map.update(
            {
                arn: _PlanqkAwsDevice(arn, planqk_client=planqk_client) #TODO use planqk clcient globally???
